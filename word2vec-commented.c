...
void *TrainModelThread(void *id) {
  ...
  while (1) {
    ...
    if (sentence_length == 0) {
      ...
      // If sub-sampling is turned on, then randomly discard frequent words by directly removing them
      // from the sentence.
      while (1) {
    ...
    if (cbow) {  //train the cbow architecture
      ...
      // NEGATIVE SAMPLING
      // From the paper, it looks like you should select either negative sampling
      // or hierarchical soft-max, instead of both.
      ...
      // b is how much of the window do we skip.
      // so that we are using randomly varying window sizes.
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        c = sentence_position - window + a;
        if (c < 0) continue;
        if (c >= sentence_length) continue;
