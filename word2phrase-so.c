// A modification of word2phrase, so that it will skip merging tokens that start with "$F"
//
...
int SearchVocab(char *word) {
  if (strlen(word) > 2) {
    if (word[0] == '$' && word[1] == 'F') {
      // this word is a function (special token), skip any combination behavior about this word.
      // returning -1 here will make TrainModel() below to skip combining unigrams into a bigram
      return -1;
    }
  }
...
void TrainModel() {
  ...
  while (1) {
    ...
    oov = 0;  // oov = 1 means output as single word
    i = SearchVocab(word);
